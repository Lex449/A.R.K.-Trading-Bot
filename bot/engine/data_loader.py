...
async def fetch_from_finnhub(symbol: str, resolution: str) -> pd.DataFrame:
    """
    Tries to fetch OHLCV data from Finnhub with specific resolution.
    """
...
async def fetch_market_data(symbol: str, chat_id: int = None) -> pd.DataFrame | None:
    """
    Attempts multiple resolutions from Finnhub, falls back to Yahoo only bei Totalausfall.
    """
    ...
    # === Yahoo Finance als letzter Notnagel ===
    try:
