"""
User Timezone Scheduler – Manages user-specific Recaps and Resets.
"""
...
from bot.utils.timezone_manager import get_user_timezone
...
def start_user_timezone_scheduler(application, chat_id: int):
    """
    Starts individual Recap & Reset Jobs for a specific user based on their timezone.
    """
    ...
    logger.info(f"📊 Scheduled Daily Recap for {chat_id} at 16:05 ({timezone}).")
    ...
    logger.info(f"📈 Scheduled Weekly Recap for {chat_id} (Friday 16:10 {timezone}).")
    ...
    scheduler.add_job(
        ...
        minute=59,
        args=[chat_id],
        id=f"daily_reset_{chat_id}",
        name=f"Daily Reset for {chat_id}",
        misfire_grace_time=300
    )
    logger.info(f"♻️ Scheduled Daily Reset for {chat_id} at 23:59 ({timezone}).")
    ...
    scheduler.add_job(
        ...
        minute=0,
        args=[chat_id],
        id=f"weekly_reset_{chat_id}",
        name=f"Weekly Reset for {chat_id}",
        misfire_grace_time=600
    )
    logger.info(f"♻️ Scheduled Weekly Reset for {chat_id} (Monday 00:00 {timezone}).")
